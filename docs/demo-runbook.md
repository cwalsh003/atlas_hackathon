...
Ticket #10. The loop itself is documented in [runbook-loop.md](runbook-loop.md); the tunnel script and Vite host config shipped in #11. Submission is due 4 pm Thursday 2026-09-24; the live loop runs Thursday night with requesters filing requests from their phones.
...
9. **Scratch request.** With the loop stopped (`touch .claude/loop-stop`, wait for `loop stopped`), file one request from the phone through the modal, confirm it appears as an issue, close it with a comment, then restart the loop. Never run `npm run smoke` or the #3/#4/#5 evidence flows while the loop runs. Screenshot the phone with the address bar visible and save it under `test-results/tunnel-phone/`.
...
11. **Viewport.** On the phone: pens are absent, the whole region is the tap target, the hint banner shows under the header when edit mode is on; at 1280px on the laptop: pens on all 15 regions. After any request ships, the change appears on the phone without a refresh (hot reload through the tunnel).
...
13. **After the demo.** `touch .claude/loop-stop` and wait for `loop stopped`; Ctrl-C `scripts/tunnel.sh`; confirm the tunnel URL no longer loads on the phone.
...
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ----------------------------------------------------- |
| 1. Small request lands on its own   | Phone taps the Open orders KPI, types "Rename the Open orders KPI to Orders on the board", Build it (a fresh request: Snowed in, Cleared, Out plowing, and the snowflake already shipped as #42, #44, #51, #45). Laptop: pill goes queued, triage, building, PR open, shipped; label changes with no reload.                                          | 368–496 s filed to shipped (#42, #44, #45, #51)               | 60–75 s: file, pill states, the label flip            |
| 2. Large request approved on camera | Phone asks for a two-region change ("Show each yeti's home town in the roster and add a Town column to the work-orders table"; the Crew rename already shipped as #49). Pill: waiting for approval. Laptop shows the plan comment; presenter runs the approve command; pill: building, PR open; presenter merges; pill: shipped, both regions change. | plan 284 s, approval to PR 421 s, merge to shipped 12 s (#49) | 75–90 s: file, plan appears, approve, PR, merge, flip |
...
Approve command for loop 2 (the human plan gate in `docs/agents/issue-tracker.md`): `gh issue edit <n> --remove-label plan-review --add-label in-progress`. Merge: `gh pr merge <pr> --squash --delete-branch`.
