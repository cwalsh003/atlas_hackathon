...
A build-time **demo mode** adds an **edit mode** toggle. In edit mode every marked **component** on the dashboard shows a pen. Clicking the pen opens a prompt tied to that component. The requester types what they want and chooses a **lane**:
...
While work happens, the component shows a status pill that walks through triage, building, PR open, and live. At the end of the demo a single **consolidation** issue turns the flagged experiments into permanent code through one human-reviewed PR. A production build has no edit mode and every flag reads false, so the untouched dashboard ships clean.

## Glossary

- **Component**: a marked region of the dashboard with a stable edit id (header, each KPI card, chart, work-orders table, each table column header, footer).
- **Request**: a prompt attached to a component by a **requester**, stored as a GitHub issue.
- **Lane**: `implement` or `vote`. Implement splits into **small** (ships without a human click) and **large** (stops at plan review).
...
- **Shipped**: the issue label that turns a flag on for everyone in demo mode.
...
5. As a requester, I want a pen on every marked component, so that I can point at exactly the thing I want changed.
6. As a requester, I want the pen to open a prompt tied to that component, so that I do not have to describe where the thing is.
...
9. As a requester, I want the issue created for my request to include my prompt, the component id, the component's current markup, and my name, so that the agent can act without asking me anything.
10. As a requester, I want the component to show a status pill after I submit, so that I can see the request move.
11. As a requester, I want the pill to link to the GitHub issue, so that anyone can inspect the real ticket.
12. As a requester, I want the pill to walk through queued, triage, building, PR open, and live, so that I know what stage the work is in.
...
17. As a demo operator, I want only one implement-lane request in flight at a time with the rest shown as queued, so that agents never collide on the same component.
...
19. As a demo operator, I want the small-lane rule written into the repository's Atlas policy, so that shipping without a human click is a documented decision rather than a bypass.
20. As a demo operator, I want the large lane to follow the standard Atlas route, so that plan approval and merge stay human actions.
...
23. As a voter, I want a vote page listing every proposal with its prompt, component, requester, and vote count, so that I can see what the room wants.
...
- **Components**: about ten marked regions, each rendered by a named component with a stable `data-edit-id`. Agents edit the component that owns the id; new ids are added only when a request creates a new region.
...
- **Flag defaults**: implement-lane flags turn on for everyone once the issue is labeled shipped. Vote-lane previews are off by default and toggleable per viewer. Production build: all off.
- **Server endpoints** (Vite middleware, one process):
  - create request: takes prompt, component id, component markup, requester name, lane; creates a GitHub issue with the `needs-triage` label plus a lane label; returns the issue number.
...
- **Issue body contract**: prompt, component id, source file hint, current outer HTML of the component, requester, lane. Title is the prompt truncated, prefixed with the component id.
...
- **Triage rule** (written into the Atlas tracker policy): an implement-lane issue is `size:small` when it touches one component, adds no data fields, and adds no dependency. Small skips plan review and the in-progress human gate: the agent implements, opens a PR, merges it, labels the issue shipped. Everything else is `size:large` and follows the standard route. Requests that cannot be understood get `needs-info`; requests outside the dashboard get `wontfix`, both with a comment explaining why.
- **Agent loop**: a Claude Code loop on the laptop polls for `needs-triage` issues, triages, then runs the small lane or the standard route. One implement-lane issue in flight at a time, oldest first.
...
- **Status pill**: polls the status endpoint every five seconds for the issues the viewer submitted or that are open on the board. Label to pill mapping: needs-triage means queued or triage, planning or in-progress means building, human-review means PR open, plan-review means waiting for approval, shipped means live, needs-info or wontfix means declined with the comment shown.
...
- Typecheck and build are verification commands on every change, and the small lane must pass both before merging. Update the testing policy doc with `npm test`, `npm run typecheck`, and `npm run build` once they exist.
- Visual proof is a screenshot of the dashboard in demo mode with pens visible, and one with a live status pill, saved under the proof folder.
...
- Editing anything other than the marked components.

## Further Notes

- Submission is due 4pm Thursday 9/24/2026 with description, screenshots, and a recording. Build order: dashboard and edit mode, then flags and the small lane end to end, then the vote page, then the large lane and recording. The cut line is after the large lane; if the vote page slips it becomes a "coming soon" tab.
